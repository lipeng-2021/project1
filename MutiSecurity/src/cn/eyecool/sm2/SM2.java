...
public class SM2 {
    ...
    public byte[] sm2GetZ(byte[] userId, ECPoint userKey) {
        ...
        byte[] p = SecurityUtils.byteConvert32Bytes(this.ecc_a);
        sm3.update(p, 0, p.length);
        
        p = SecurityUtils.byteConvert32Bytes(this.ecc_b);
        sm3.update(p, 0, p.length);
        
        p = SecurityUtils.byteConvert32Bytes(this.ecc_gx);
        sm3.update(p, 0, p.length);
        
        p = SecurityUtils.byteConvert32Bytes(this.ecc_gy);
        sm3.update(p, 0, p.length);
        
        p = SecurityUtils.byteConvert32Bytes(userKey.normalize().getXCoord().toBigInteger());
        sm3.update(p, 0, p.length);
        
        p = SecurityUtils.byteConvert32Bytes(userKey.normalize().getYCoord().toBigInteger());
        sm3.update(p, 0, p.length);
